...
'''
python execute.py --testtype instrument --apk 'notepad.apk' --package "com.anoid.notepad" --testapk "rt_NotePad.apk" --testpackage "com.notepadblackboxtest" --runner "android.test.InstrumentationTestRunner"
'''
def Start(products,filename, decodeJson=None,items=None):
    Instrument(products,filename,decodeJson = decodeJson, items = items).run()
def CreateItem(cmdparams):
    item = InstrumentItem()
    item.itemtype = 'instrument'
    item.itemName = 'instrument'
    item.apkName = cmdparams.apk and cmdparams.apk or ''
    item.package = cmdparams.package and cmdparams.package or ''
    item.testapk = cmdparams.testapk and cmdparams.testapk or ''
    item.testpackage = cmdparams.testpackage and cmdparams.testpackage or ''
    item.runner = cmdparams.runner and cmdparams.runner or ''
